...
def test_cycle_one_job(jm, sample_data,
    ...
    remaining_line_ids = set(item.id for item in failure_lines_remaining)
    assert set(item.id for item in FailureLine.objects.all()) == remaining_line_ids

    assert set(int(item.meta.id) for item in TestFailureLine.search().execute()) == remaining_line_ids
